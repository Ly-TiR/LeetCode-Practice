...
function maxOperations(nums: number[]): number {
  ...
  const dfs = (left: number, right: number, target: number): number => {
  ...
  let result1:number = dfs(2, n - 1, nums[0] + nums[1]);
  let result2:number = dfs(0, n - 3, nums[n - 1] + nums[n - 2]);
  let result3:number = dfs(1, n - 2, nums[0] + nums[n - 1]);
